...
router.put("/read/:notification_id", async (req: Request, res) => {
  const decodedToken = req.decodedIdToken;
  const notification_id = req.params?.notification_id;
  const db = req.app.get("db") as PoolClient
  try {
    await db.query(`UPDATE notifications SET is_read = true WHERE user_id = $1 AND id = $notification_id`, [decodedToken?.db_user_id, notification_id]);
